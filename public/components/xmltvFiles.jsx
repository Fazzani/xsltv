    ...
    return (
      <div className="xviewer-list">
        {this.props.files.map((file, key) => {
          return <XmltvFileItem file={file} key={key} itemCallback={this.props.onSettingsModalCallback} />;
        })}
        <XmltvFileItem isNew={true} itemCallback={this.props.onSettingsModalCallback} />
      </div>
    );
  }
}
...
class XmltvFileItem extends Component {
  ...
  render() {
    this.state.isNew = !this.props.file ? true : this.props.isNew;

    return (
      <div className="row xviewer-list-item">
        {this.state.isNew ? (
          <div className="col">
            <input type="url" value={this.state.value || ''} onChange={e => this.handleChange(e)} placeholder="Add new xmltv url..." className="form-control" />
          </div>
        ) : (
