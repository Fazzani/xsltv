...
export default class Home extends React.PureComponent<HomeProps, HomeState> {
  ...
  static FallbackSelectedFileUrl =
    "https://raw.githubusercontent.com/Fazzani/grab/master/ar_bein.xmltv";
  ...
  componentWillReceiveProps(nextProps: HomeProps) {
    if (JSON.stringify(nextProps.files) !== JSON.stringify(this.props.files)) {
      const selectedFile = (nextProps.files &&
        nextProps.files.find(f => f.selected === true)) || {
        url: Home.FallbackSelectedFileUrl,
        name: "Default"
      };
      this.setState(
        { files: nextProps.files, selectedFile: selectedFile },
        () => this.loadFile()
      );
    }
  }
  ...
  async componentDidMount() {
    ...
    const selectedFile = (this.props.files &&
      this.props.files.find(f => f.selected === true)) || {
      url: Home.FallbackSelectedFileUrl,
      name: "Default"
    };
  ...
  async loadFile() {
    if (this.state.selectedFile) {
      this.context.toggleLoader(
        `Downloading the file ${this.state.selectedFile.name}...`,
        true
      );
      ...
      if (docJson.tv && docJson.tv.programme) {
        docJson.tv.programme.map((p: Program) => {
          p.startTime = DateTime.fromFormat(
            p.start,
            Constants.OTHERS.XMLTV_DATETIME_FORMAT
          );
          p.stopTime = DateTime.fromFormat(
            p.stop,
            Constants.OTHERS.XMLTV_DATETIME_FORMAT
          );

          p.duration = Interval.fromDateTimes(p.startTime, p.stopTime).length(
            "minute"
          );
        ...
        const allTvgChannels = docJson.tv.channel.map((c: Channel) => {
          c.programs = docJson.tv.programme.filter(
            (p: Program) => p.channel === c.id
          );
          return c;
        });
  ...
  fetchPrograms = () => {
    ...
    this.state.allTvgChannels.forEach((c: Channel) => {
      ...
      ch.programs = c.programs
        .filter((p: Program) => {
          return (
            (p.startTime.diff(currentDay, "minutes").minutes >= 0 &&
              p.startTime.diff(currentDay, "minutes").minutes <=
                MinutesPerDay) ||
            (p.stopTime.diff(currentDay, "minutes").minutes >= 0 &&
              p.stopTime.diff(currentDay, "minutes").minutes <= MinutesPerDay)
          );
        })
        .slice();
          ...
          {INTERVALS.map(x => {
            return (
              <div
                ...
                style={{ width: this.state.halfHourWidth }}
                key={x}
              >
                {x}
              </div>
        ...
        return (
          <li key={c.id} className="listings-channel-row">
            <div
              ...
              style={{ width: this.state.channelLeftWidth }}
            >
        ...
        return (
          ...
          >
            ...
            <div className="listings-details">
              <span className="listings-details-first">
    ...
    const navigationButtons = (
      <div className="toolbar">
        ...
        <a
          ...
          onClick={e => this.onSlide(e, false)}
          data-toggle="tooltip"
          data-placement="top"
          title="Next"
        >
        ...
        {this.state && (
          ...
          >
            {this.state.selectedChannel && (
              <TvgChannel channel={this.state.selectedChannel} />
            )}
          </SidePanel>
        )}
      </React.Fragment>
