...
export default class MissedPrograms extends React.Component<
  {},
  MissedProgramsState
> {
  ...
  render() {
    ...
    const ListChannels = (props: any) => {
      return props &&
        props.value &&
        props.value.inner_hits.channels_without_epg ? (
        props.value.inner_hits.channels_without_epg.hits.hits.map(
          (channel: any, i: number) => (
            <div
              className={
                "card text-white bg-primary mb-3 " +
                (channel._source.active ? "enabled" : "disabled")
              }
